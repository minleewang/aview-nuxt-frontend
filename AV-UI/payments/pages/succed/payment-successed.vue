    ...
    <div class="box_section" style="width: 600px">
      ...
      <div>
        ...
        <button
          class="button p-grid-col5"
          @click="goToOrderList"
          style="background-color: #d1e7ff; color: #0a58ca; margin-left: 10px"
        >
          주문 목록 보기
        </button>
      </div>
...
definePageMeta({
  ...
  description:
    "잡스틱(JobStick) 결제하기가 성공되었습니다. 결제 정보를 확인하세요.",
  keywords: [
    "결제",
    "결제 성공",
    "결제 정보",
  ...
  ogDescription:
    "잡스틱(JobStick) 결제하기 성공 페이지입니다. 결제 정보를 확인하세요.",
  ogImage: "",
...
async function confirm(requestData) {
  try {
    ...
    if (!response.ok) {
      router.push(`/payments/fail?message=${json.message}&code=${json.code}`);
    } else {
