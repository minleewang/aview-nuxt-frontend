      ...
      >
        ...
        >
          ...
          <v-card-subtitle class="text-h6 font-weight-bold mb-4 text-center">
            <span v-if="plan.originalPrice" class="original-price">{{ plan.originalPrice }}원</span>
            <span class="discounted-price">{{ plan.price }}원</span> / {{ plan.period }}
          </v-card-subtitle>
          ...
          <v-card-actions>
            <v-btn block color="white" class="text-primary font-weight-bold" variant="flat" @click="subscribe(plan.id)">
              구독하기
            </v-btn>
          </v-card-actions>
        </v-card>
