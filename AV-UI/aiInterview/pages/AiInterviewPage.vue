<template>
  <div class="home-container">
    <div class="content-wrapper">
      <h2 class="prompt-title">AIV AI INTERVIEW</h2>
      <h4 class="prompt-subtitle">AIV에서 제공하는 AI 인성면접 서비스에요</h4>
...
const chatHistory = reactive([
  {
    type: "ai",
    content:
      '안녕하세요! AI 모의 면접 서비스입니다. "면접 시작"을 입력하시면 면접을 시작할 수 있습니다. 총 5번의 질문이 제공됩니다.',
  },
]);
  ...
  if (userInput.value.trim()) {
    ...
    setTimeout(async () => {
      if (aiResponseList.length === 0) {
        ...
        aiResponseList.push(
          ...(await aiInterviewStore.requestGetQuestionListToDjango({
            sessionId,
          }))
        );
      }
      const aiResponse =
        aiResponseList.questionList[questionIndex.value] ||
        "면접이 종료되었습니다.";
      questionIndex.value++;
      chatHistory.push({
        type: "ai",
