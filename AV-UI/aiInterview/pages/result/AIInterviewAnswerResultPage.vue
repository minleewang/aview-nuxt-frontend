    ...
    <v-container width="60%">
      ...
      <v-row v-for="(item, index) in inputList" :key="index" class="my-4">
        <v-col cols="12" class="card">
          <p>
            <strong>{{ index + 1 }}. {{ item[0] }}</strong>
          </p>
          <v-col cols="12">
            <li><strong>당신의 답변📄</strong></li>
            <p class="gray-text">{{ item[1] }}</p>
            <br />
            <li>
              <strong>AI 평가 결과 🪄</strong>
              <v-col cols="12" class="card2">
                ...
                <v-text
                  ><strong>피드백</strong>
                  <p>
                    해당 질문의 의도는 당신의 <strong>{{ item[2] }}</strong
                    >을 파악하기 위함입니다.
                  </p>
                  <p>{{ item[4] }}</p>
                </v-text>
...
const splitScoreFeedback = (feedback) => {
  ...
  if (feedbackParts.length !== 1) {
    return [
      feedbackParts[0].replace("score:", ""),
      feedbackParts[1].replace("feedback:", ""),
    ];
  } else {
    return [
      "문제 발생",
