...
const setRedirectData = async () => {
  try {
    ...
    await naverAuthenticationStore.requestNaverAccessTokenToDjangoRedirection({
      code,
    });
    ...
    const isEmailDuplication =
      await accountStore.requestEmailDuplicationCheckToDjango({ email: email });
    ...
    if (isEmailDuplication === true) {
      ...
      if (roleType.data.roleType === "ADMIN") {
        ...
        naverAuthenticationStore.isAuthenticated = false; // 구글 인증 상태 해제
