...
export const naverAuthenticationAction = {
  async requestNaverOauthRedirectionToDjango(): Promise<void> {
    ...
    try {
      return djangoAxiosInstance.get("/naver_oauth/naver").then((res) => {
        window.location.href = res.data.url;
      });
    } catch (error) {
      console.error(
        "requestNaverOauthRedirectionToDjango() 중 오류 발생",
        error
      );
      throw error;
    }
  ...
  async requestNaverAccessTokenToDjangoRedirection(
    ...
    try {
      const response = await djangoAxiosInstance.post(
        "/naver_oauth/naver/access-token",
        code
      );
  ...
  async requestAddNaverRedisAccessTokenToDjango(payload: {
    email: string;
    accessToken: string;
  }): Promise<any> {
    const { djangoAxiosInstance } = axiosUtility.createAxiosInstances();
  ...
  async requestNaverLogoutToDjango(userToken: string): Promise<void> {
    ...
    try {
      ...
      const res = await djangoAxiosInstance.post("/naver_oauth/logout", {
        userToken: userToken,
      });
