<template>
  <v-container>
    <br /><br /><br />
    <v-card class="mx-auto my-12" max-width="800" v-if="reviewForm">
      <v-card-title>
        <span class="headline">{{ reviewForm.reviewTitle }}</span>
      </v-card-title>
      <v-card-subtitle>
        <span>{{ reviewForm.reviewDescription }}</span>
      </v-card-subtitle>
      <v-card-text>
        <v-form ref="form" v-model="valid" lazy-validation>
          <v-row
            v-for="(question, index) in reviewForm.reviewQuestions"
            :key="index"
            class="mb-4"
          >
            <v-col cols="12">
...
const reviewForm = computed(() => reviewStore.reviewForm);
...
const isChecked = (index, option) => {
  return reviewForm.value.reviewQuestions[index].answer.includes(option);
};

const toggleCheckbox = (index, option, questionType, questionId) => {
  const answerArray = reviewForm.value.reviewQuestions[index].answer;
...
const updateCheckboxSubmitForm = (answerArray, questionType, questionId) => {
  submitForm.value = submitForm.value.filter(
...
const onSubmit = async () => {
  let isValid = true;
  reviewForm.value.reviewQuestions.forEach((question) => {
    if (
      question.essential &&
  ...
  if (isValid) {
    ...
    const isSubmitted = await reviewStore.requestSubmitReviewToDjango(
      payload.value
    );
    if (isSubmitted) {
      alert("제출이 완료되었습니다.");
...
onMounted(async () => {
  if (email.value) {
    ...
    isFirstAccept.value = await reviewStore.requestCheckIsFirstSubmit({
      accountId: accountId.value,
    });
  ...
  await reviewStore.requestReviewReviewFormToDjango(randomString.value);
});
</script>
