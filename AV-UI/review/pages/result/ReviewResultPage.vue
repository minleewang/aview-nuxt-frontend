<template>
  ...
  >
    <v-card-title>
      <span class="headline">{{ resultForm.reviewTitle }}</span>
    </v-card-title>
    <v-card-subtitle>
      <span>{{ resultForm.reviewDescription }}</span>
    </v-card-subtitle>
    <v-card-text>
      <v-row
        v-for="(question, index) in resultForm.reviewQuestions"
        :key="index"
        class="mb-4"
      >
        <v-col cols="12">
...
const { resultForm } = toRefs(reviewStore);
...
onMounted(async () => {
  await reviewStore.requestReviewResultToDjango(reviewId.value);

  resultForm.value.reviewQuestions.forEach((question, index) => {
    if (
      question.questionType === "radio" ||
      question.questionType === "checkbox"
    ) {
