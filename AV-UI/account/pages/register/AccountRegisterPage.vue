...
const requestUserInfo = async () => {
  try {
    if (loginType.value === "KAKAO") {
      const kakaoUserInfo =
        await kakaoAuthenticationStore.requestUserInfoToDjango();
      kakaoEmail.value = kakaoUserInfo.kakao_account.email;
    } else if (loginType.value === "GOOGLE") {
      const googleUserInfo =
        await googleAuthenticationStore.requestGoogleUserInfoToDjango();
      googleEmail.value = googleUserInfo.email;
    } else if (loginType.value === "NAVER") {
      const naverUserInfo =
        await naverAuthenticationStore.requestNaverUserInfoToDjango();
...
const checkNicknameDuplication = async () => {
  try {
    const isDuplicate =
      await accountStore.requestNicknameDuplicationCheckToDjango({
...
const submitForm = async () => {
  if (isValidForSubmission.value) {
    ...
    if (accessToken) {
      if (loginType.value === "KAKAO") {
        await kakaoAuthenticationStore.requestAddRedisAccessTokenToDjango({
          email: accountInfo.email,
          accessToken,
        });
      } else if (loginType.value === "GOOGLE") {
        await googleAuthenticationStore.requestAddGoogleRedisAccessTokenToDjango(
          { email: accountInfo.email, accessToken }
        );
      } else if (loginType.value === "NAVER") {
        await naverAuthenticationStore.requestAddNaverRedisAccessTokenToDjango({
          email: accountInfo.email,
          accessToken,
        });
      }
...
.register-container {
  ...
  padding: 0;
  background: url("@/assets/images/fixed/login_bg61.jpg") no-repeat center center; /* 배경 이미지 설정 */
...
:deep(.v-field--error:not(.v-field--disabled) .v-label.v-field-label) {
  color: rgba(
    ...
    55,
    0.75
  ) !important; /* 에러 상태의 레이블 색상을 초록색으로 변경 */
}
</style>
