...
export const kakaoAuthenticationAction = {
    async requestKakaoLoginToDjango(): Promise<void> {
        ...
        try {
            return djangoAxiosInst.get('/kakao-oauth/request-login-url').then((res) => {
                console.log(`res: ${res}`)
    ...
    async requestAccessToken(code: string): Promise<string | null> {
        const { djangoAxiosInst } = axiosUtility.createAxiosInstances();
        try {
            const response = await djangoAxiosInst.post('/kakao-oauth/redirect-access-token', code)
    ...
    async requestValidationUserToken(userToken: string): Promise<boolean> {
        ...
        try {
            const response = await djangoAxiosInst.post('/kakaoAuthentication/validation', { userToken })
